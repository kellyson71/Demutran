...
<div class="bg-white shadow rounded-lg p-6">
    ...
    <div class="border-b pb-6 mb-6">
        ...
        <div class="space-y-4">
            <?php
                echo createEditableField('Nome', $recurso['nome'] ?? 'Não informado', 'nome');
                echo createEditableField('Tipo de Requerente', ucfirst($recurso['tipo_requerente'] ?? 'Não informado'), 'tipo_requerente');
                echo createEditableField('CPF', $recurso['cpf'] ?? 'Não informado', 'cpf');
                echo createEditableField('Email', $recurso['gmail'] ?? 'Não informado', 'gmail');
                echo createEditableField('Telefone', $recurso['telefone'] ?? 'Não informado', 'telefone');
                echo createEditableField('Endereço', $recurso['endereco'] ?? 'Não informado', 'endereco');
                echo createEditableField('Número', $recurso['numero'] ?? 'Não informado', 'numero');
                echo createEditableField('Bairro', $recurso['bairro'] ?? 'Não informado', 'bairro');
                echo createEditableField('CEP', $recurso['cep'] ?? 'Não informado', 'cep');
                ?>
        </div>
    </div>
    ...
    <div class="border-b pb-6 mb-6">
        ...
        <div class="space-y-4">
            <?php
                echo createEditableField('Placa', $recurso['placa'] ?? 'Não informado', 'placa');
                echo createEditableField('Marca/Modelo', $recurso['marcaModelo'] ?? 'Não informado', 'marcaModelo');
                echo createEditableField('Cor', $recurso['cor'] ?? 'Não informado', 'cor');
                echo createEditableField('Espécie', $recurso['especie'] ?? 'Não informado', 'especie');
                echo createEditableField('Categoria', $recurso['categoria'] ?? 'Não informado', 'categoria');
                echo createEditableField('Ano', $recurso['ano'] ?? 'Não informado', 'ano');
                ?>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <p class="text-gray-600"><span class="font-semibold">Auto de Infração:</span>
                    <?php echo htmlspecialchars($recurso['autoInfracao'] ?? 'Não informado'); ?></p>
                <p class="text-gray-600"><span class="font-semibold">Data da Infração:</span>
                    <?php echo $recurso['dataInfracao'] ? date('d/m/Y', strtotime($recurso['dataInfracao'])) : 'Não informado'; ?>
                </p>
            </div>
            <div>
                <p class="text-gray-600"><span class="font-semibold">Local da Infração:</span>
                    <?php echo htmlspecialchars($recurso['localInfracao'] ?? 'Não informado'); ?></p>
                <p class="text-gray-600"><span class="font-semibold">Enquadramento:</span>
                    <?php echo htmlspecialchars($recurso['enquadramento'] ?? 'Não informado'); ?></p>
            </div>
        </div>
